...
class DatabaseManager:
    ...
    def is_healthy(self) -> bool:
        """检查所有数据库连接是否健康"""
        redis_ready = (not settings.REDIS_ENABLED) or self._redis_healthy
        return self._mongo_healthy and redis_ready
    ...
    try:
        ...
        if settings.REDIS_ENABLED:
            try:
                ...
                logger.warning(f"⚠️ Redis不可用，系统将以降级模式启动: {e}")
