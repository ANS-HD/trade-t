...
class AnalysisService:
    ...
    async def submit_single_analysis(
        ...
        try:
            ...
            # 应用系统级并发与可见性超时（若提供）
            try:
                configured_limit = int(effective_settings.get("max_concurrent_tasks", DEFAULT_USER_CONCURRENT_LIMIT))
                self.queue_service.user_concurrent_limit = min(configured_limit, DEFAULT_USER_CONCURRENT_LIMIT)
                self.queue_service.global_concurrent_limit = min(configured_limit, GLOBAL_CONCURRENT_LIMIT)
