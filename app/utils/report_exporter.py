...
try:
    ...
    logger.warning(f"⚠️ WeasyPrint 未安装，PDF 导出兜底不可用: {e}")
except Exception as e:
    WEASYPRINT_ERROR = str(e)
    logger.warning(f"⚠️ WeasyPrint 检测失败: {e}")

# 检查 ReportLab（纯 Python PDF 兜底）
...
try:
    ...
    logger.info("✅ ReportLab 可用（PDF 生成工具）")
except ImportError as e:
    REPORTLAB_ERROR = str(e)
    logger.warning(f"⚠️ ReportLab 未安装，PDF 纯 Python 兜底不可用: {e}")
except Exception as e:
    REPORTLAB_ERROR = str(e)
    logger.warning(f"⚠️ ReportLab 检测失败: {e}")
...
class ReportExporter:
    ...
    def __init__(self):
        ...
        self.pdf_available = (
            self.pdfkit_available
            or self.weasyprint_available
            or self.reportlab_available
        )
        ...
        logger.info(f"  - reportlab_available: {self.reportlab_available}")
        logger.info(f"  - pdf_available: {self.pdf_available}")
...
"""
    ...
    def _generate_pdf_with_reportlab(self, md_content: str) -> bytes:
        """使用 ReportLab 从 Markdown 文本生成简洁 PDF。"""
        logger.info("🔧 使用 ReportLab 生成 PDF...")
        ...
        for raw_line in md_content.splitlines():
            ...
            if safe_line.startswith("# "):
                story.append(Paragraph(safe_line[2:], heading_style))
            else:
        ...
        logger.info(f"✅ ReportLab PDF 生成成功，大小: {len(pdf_bytes)} 字节")
        return pdf_bytes

    def generate_pdf_report(self, report_doc: Dict[str, Any]) -> bytes:
        """生成 PDF 格式报告。优先使用 pdfkit，失败时回退到 WeasyPrint。"""
        logger.info("📊 开始生成 PDF 文档...")

        # 检查是否存在任何可用的 PDF 生成工具
        if not self.pdf_available:
            error_msg = (
                "PDF 导出功能不可用。\n\n"
                "安装方法:\n"
                "1. 安装 wkhtmltopdf 并保留 pdfkit\n"
                "2. 或安装 WeasyPrint: pip install weasyprint\n"
                "3. 或安装 ReportLab: pip install reportlab\n"
            )
            if PDFKIT_ERROR:
        ...
        try:
            ...
            return self._generate_pdf_with_reportlab(md_content)
        except Exception as e:
            if self.pdfkit_available and self.weasyprint_available:
                logger.warning(f"⚠️ pdfkit 生成失败，尝试使用 WeasyPrint: {e}")
                try:
                    return self._generate_pdf_with_weasyprint(html_content)
                except Exception as fallback_error:
                    if self.reportlab_available:
                        logger.warning(f"⚠️ WeasyPrint 生成失败，尝试使用 ReportLab: {fallback_error}")
                        try:
                            return self._generate_pdf_with_reportlab(md_content)
                        except Exception as reportlab_error:
                            error_msg = f"PDF 生成失败: {reportlab_error}"
                            logger.error(f"❌ {error_msg}")
                            raise Exception(error_msg)
                    error_msg = f"PDF 生成失败: {fallback_error}"
                    logger.error(f"❌ {error_msg}")
                    raise Exception(error_msg)

            if self.reportlab_available:
                logger.warning(f"⚠️ 主 PDF 引擎失败，尝试使用 ReportLab: {e}")
                try:
                    return self._generate_pdf_with_reportlab(md_content)
                except Exception as reportlab_error:
                    error_msg = f"PDF 生成失败: {reportlab_error}"
